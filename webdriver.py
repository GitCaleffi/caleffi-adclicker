...
def create_webdriver(
    ...
    if proxy:
        ...
        if config.webdriver.language_from_proxy:
            ...
            # Merge with existing webrtc_preferences to avoid overwriting them
            merged_prefs = {**webrtc_preferences, "intl.accept_languages": str(lang)}
    ...
    # --- PROXY VERIFICATION: check what IP Chrome actually uses ---
    try:
        driver.get("https://api.ipify.org")
        chrome_ip = driver.find_element("tag name", "body").text.strip()
        logger.info(f"[PROXY CHECK] Chrome external IP: {chrome_ip}")
        # check if it matches the proxy IP we resolved earlier
        try:
            geo_resp = requests.get("http://ip-api.com/json/" + chrome_ip, timeout=5)
            geo = geo_resp.json()
            logger.info(
                f"[PROXY CHECK] Chrome IP location: {geo.get('city')}, {geo.get('regionName')}, {geo.get('country')} | ISP: {geo.get('isp')}"
            )
        except Exception:
            pass
    except Exception as e:
        logger.warning(f"[PROXY CHECK] Could not verify Chrome IP: {e}")
