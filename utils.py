...
def get_location(geolocation_db_client: GeolocationDB, proxy: str) -> tuple[float, float, str, str]:
    ...
    if config.webdriver.auth:
        for repeat in range(2):
            try:
                response = requests.get("https://api.ipify.org", proxies=proxies_header, timeout=5, verify=False)
                ...
                try:
                    ...
                    response = requests.get(
                        "https://ipv4.webshare.io/", proxies=proxies_header, timeout=5, verify=False
                    )
                    ip_address = response.text
                    ...
                    try:
                        ...
                        response = requests.get(
                            "https://ipconfig.io/json", proxies=proxies_header, timeout=5, verify=False
                        )
    ...
    if db_result:
        ...
        if not country_code:
            try:
                ...
                try:
                    response = requests.get(
                        "https://ifconfig.co/json", proxies=proxies_header, timeout=5, verify=False
                    )
        ...
        while retry_count < max_retry_count:
            try:
                ...
                if not (latitude and longitude and country_code):
                    raise Exception("Failed with http://ip-api.com")
...
def add_cookies(driver: undetected_chromedriver.Chrome) -> None:
    ...
    # Only skip cookies that encode precise location (DV = lat/lon, NID = user prefs with location)
    # All other cookies (AEC, SOCS, __Secure-STRP, __Secure-BUCKET) are kept so the browser
    # looks like an existing session and avoids captcha
    location_cookies = {"NID", "DV", "1P_JAR", "UULE"}

    for cookie in cookies:
        if cookie.get("name") in location_cookies:
            logger.debug(f"Skipping location cookie '{cookie.get('name')}'")
            continue
        ...
        # Rimuove campi non standard e converte expirationDate → expiry
        clean_cookie = {
        ...
        try:
            ...
            logger.debug(f"Skipping cookie '{cookie.get('name')}': {exp}")
